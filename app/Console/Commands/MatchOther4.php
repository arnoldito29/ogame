...
class MatchOther4 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'get:match-other4';
    ...
    public function handle()
    {
        ...
        $teams = $this->teamService->team::where('id', '>', 126)->get();
        $total = 0;
        foreach ($teams as $team) {
            ...
            foreach ($match as $matchItem) {
                $team1 = $split . '_1';
                $team2 = $split . '_2';
                if ($matchItem->coff1 > $matchItem->coff3 && $matchItem->$team1 < $matchItem->$team2) {
                    $this->showInfo($overtime);
                    $overtime = 0;
                    $total++;
                } else if ($matchItem->coff1 <= $matchItem->coff3 && $matchItem->$team1 > $matchItem->$team2) {
                    $this->showInfo($overtime);
                    $overtime = 0;
                    $total++;
                } else {
            ...
            $this->showInfo($overtime);
            $this->showInfo('team: '. $team->id. ' total: ' . $total . '--' . $matchItem->id);
        }
